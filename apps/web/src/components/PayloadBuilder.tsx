...
export default function PayloadBuilder() {
  ...
  const [activeTab, setActiveTab] = useState<'system' | 'history' | 'rag' | 'tools' | 'user'>('system');
  ...
  const toolOutputs = payloadSections.filter(s => s.type === 'tool_output');

  // ==============================================================================
  // EDUCATIONAL SYSTEM DESIGN NOTE: 300MS CLIENT-SIDE DEBOUNCE HOOK
  // To avoid hammering backend API analysis with network requests on every individual
  // keystroke, we hold edit text states locally in React state and broadcast updates
  // to the Zustand store only after a 300ms period of typing inactivity.
  // ==============================================================================
  ...
  useEffect(() => {
    if (systemPrompt) setLocalSysText(systemPrompt.content);
    if (activeInput) setLocalUserText(activeInput.content);
  }, [payloadSections]);
  ...
  // Tool outputs adding states
  const [newToolTitle, setNewToolTitle] = useState('');
  const [newToolContent, setNewToolContent] = useState('');

  const handleAddToolOutput = () => {
    if (!newToolContent.trim()) return;
    
    const toolIndex = toolOutputs.length + 1;
    addPayloadSection({
      id: `sec_tool_${Date.now()}`,
      type: 'tool_output',
      title: newToolTitle.trim() || `Tool JSON response #${toolIndex}`,
      content: newToolContent,
      priority: 6, // Default priority for tool executions
      required: false
    });
    ...
    <div className="glass-card p-6 rounded-3xl w-full flex flex-col gap-6 h-full grow">
      ...
      <div className="flex gap-1.5 p-1 rounded-2xl bg-slate-100 border border-slate-200">
        <button
          ...
          className={`flex items-center gap-2 px-4 py-2.5 rounded-xl font-bold text-xs transition-all duration-150 grow ${
        ...
        <button
          ...
          className={`flex items-center gap-2 px-4 py-2.5 rounded-xl font-bold text-xs transition-all duration-150 grow ${
            activeTab === 'tools' ? 'bg-white text-indigo-900 shadow-sm border border-slate-200/50' : 'text-slate-500 hover:text-slate-800'
          }`}
        >
          <Braces className="w-3.5 h-3.5" /> Tool Outputs ({toolOutputs.length})
        </button>
        <button
        ...
        >
          <Sparkles className="w-3.5 h-3.5" /> Active Input
        </button>
      </div>
      ...
      <div className="flex-1 flex flex-col min-h-[380px] justify-between">
        ...
        {/* T2: CONVERSATION TURN SIMULATOR */}
        {activeTab === 'history' && (
                ...
                historyTurns.map((turn, i) => (
                  <div key={turn.id} className="flex gap-3 items-start p-3 bg-slate-50 border border-slate-200 rounded-2xl relative group">
                    ...
                    <button
                      ...
                      className="absolute right-3 top-3.5 text-slate-400 hover:text-rose-500 opacity-0 group-hover:opacity-100 transition-opacity duration-150"
                    >
        ...
        {/* T3: VECTOR SEARCH DOCUMENT INGEST */}
        {activeTab === 'rag' && (
          <div className="flex flex-col gap-4 grow">
        ...
        {/* T4: TOOL OUTPUT JSON INGEST */}
        {activeTab === 'tools' && (
          <div className="flex flex-col gap-4 grow">
            {/* Scrollable list of tool outputs */}
            <div className="flex flex-col gap-2 max-h-[200px] overflow-y-auto pr-1">
              {toolOutputs.length === 0 ? (
                <div className="text-center py-8 text-xs font-bold text-slate-400 italic">
                  No simulated JSON tool outputs injected yet.
                </div>
              ) : (
                toolOutputs.map((tool) => (
                  <div key={tool.id} className="flex gap-3 items-start p-3 bg-slate-50 border border-slate-200 rounded-2xl relative group">
                    <div className="p-2 rounded-xl bg-cyan-500 text-white">
                      <Braces className="w-3.5 h-3.5" />
                    </div>
                    <div className="grow">
                      <div className="flex items-center justify-between">
                        <span className="text-xs font-black text-slate-700 leading-none">{tool.title}</span>
                        <span className="text-[10px] text-indigo-600 font-extrabold pr-6">{tool.tokenCount} tokens</span>
                      </div>
                      <pre className="text-[11px] text-cyan-800 font-mono mt-1.5 p-2 bg-cyan-50/40 rounded-xl border border-cyan-100/50 overflow-x-auto">
                        {tool.content}
                      </pre>
                    </div>
                    <button
                      onClick={() => removePayloadSection(tool.id)}
                      className="absolute right-3 top-3.5 text-slate-400 hover:text-rose-500 opacity-0 group-hover:opacity-100 transition-opacity duration-150"
                    >
                      <Trash2 className="w-4 h-4" />
                    </button>
            ...
            {/* Ingestion builder for new JSON */}
            <div className="p-4 bg-slate-100 rounded-2xl border border-slate-200 flex flex-col gap-3">
              <input
                type="text"
                value={newToolTitle}
                onChange={(e) => setNewToolTitle(e.target.value)}
                placeholder="Tool Name / API endpoint log (e.g. GET /api/cart/102)"
              ...
              <div className="flex gap-2">
                <input
                  ...
                  placeholder='Paste API JSON log string... (e.g. {"status": "success"})'
                  className="grow px-4 py-2.5 text-xs rounded-xl custom-input"
                />
                <button
                  onClick={handleAddToolOutput}
                  className="px-4 bg-cyan-500 hover:bg-cyan-600 text-white rounded-xl text-xs font-bold flex items-center gap-2 cursor-pointer shadow-sm transition-colors"
                >
        ...
        {activeTab === 'user' && (
          <div className="flex flex-col gap-4">
            <div className="flex justify-between items-center bg-teal-50 border border-teal-100 p-4 rounded-2xl text-teal-900">
              <span className="text-xs font-semibold leading-tight block">
                The **Active Immediate User Input** represents the incoming question targeted for evaluation. 
                Like the System Prompt, this segment is marked `required: true` and is **100% immune from prunings**.
              </span>
            </div>
            <textarea
              value={localUserText}
